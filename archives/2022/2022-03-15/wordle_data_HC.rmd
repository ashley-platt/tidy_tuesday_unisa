...
tweets <- read.csv("D:\\Tidy_Tuesday\\Wordle\\2022-03-15\\tweets.csv")
...
```{r combine with tweet data and clean}
...
wordle_all <- wordle_tweets %>% 
  mutate(letters = answer) %>% # v split words into letters v
  separate(letters, into = c("L0", "L1", "L2", "L3", "L4", "L5"), sep = "") %>% 
  select(-L0) %>% 
  mutate(L1F = letter_frequency(L1), # assign letter frequency values 
         L2F = letter_frequency(L2),
         L3F = letter_frequency(L3),
         L4F = letter_frequency(L4),
         L5F = letter_frequency(L5),
         letter_freq_total = L1F + L2F + L3F + L4F + L5F) %>% # add LF values
  group_by(wordle_id) %>% 
...
write.csv(wordle_all, "D:\\Tidy_Tuesday\\Wordle\\2022-03-15\\wordle_all.csv", row.names = FALSE)

# simplified dataset with only group data, not individual 
write.csv(wordle_words, "D:\\Tidy_Tuesday\\Wordle\\2022-03-15\\wordle_words.csv", row.names = FALSE)

# dataset with only individual data, not group
write.csv(wordle_tweets, "D:\\Tidy_Tuesday\\Wordle\\2022-03-15\\wordle_tweets.csv", row.names = FALSE)
